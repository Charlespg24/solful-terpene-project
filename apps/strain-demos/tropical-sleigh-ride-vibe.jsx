...
const TERPENES = [
  { name: "β-Ocimene", pct: 0.71, ratio: 30, color: "#B088D4", oil: "Basil", food: "Basil, Mint, Lavender, Mango", note: "Rare, bright herbal molecule. Vivid uplift and present clarity." },
  { name: "β-Caryophyllene", pct: 0.70, ratio: 30, color: "#E8A87C", oil: "Black Pepper", food: "Black Pepper, Cloves, Cinnamon, Oregano", note: "CB2 agonist — dietary cannabinoid. Warm, grounded spice." },
  { name: "α-Humulene", pct: 0.28, ratio: 12, color: "#A8945A", oil: "Hops (shared)", food: "Hops, Sage, Ginseng, Ginger", note: "Earthy woody balance. Cannabis and hops are botanical cousins." },
  { name: "Limonene", pct: 0.22, ratio: 9, color: "#F4D35E", oil: "Grapefruit", food: "Citrus Peels, Juniper, Dill", note: "Mood elevation and stress relief. Rapid absorption." },
];

const EO_BLEND = [
  { name: "Basil", drops: 14, terpene: "β-Ocimene", color: "#B088D4" },
  { name: "Black Pepper", drops: 12, terpene: "β-Caryophyllene", color: "#E8A87C" },
  { name: "Peppermint", drops: 6, terpene: "Menthol / β-Ocimene", color: "#7ECBA1" },
  { name: "Grapefruit", drops: 4, terpene: "Limonene", color: "#F4D35E" },
  { name: "Geranium", drops: 4, terpene: "Geraniol / Linalool", color: "#C4A8D4" },
];

const BLEND_RATIONALE = "Basil delivers the rare ocimene (35% of blend) that defines this strain's unique character. Black pepper for the heavy caryophyllene. Peppermint captures the distinctive aroma note. Grapefruit adds citrus brightness. Geranium brings the honeysuckle floral quality.";

const PROTOCOL_STEPS = [
  { time: "T – 30 min", label: "Diffuse", desc: "Start diffusing basil + black pepper. Open space, natural daylight, 4200K bright accent lighting.", icon: "☀" },
  { time: "T – 15 min", label: "Sip", desc: "Pour the Mint Lift mocktail or Clarity Blend juice. Mint and ginger deliver through the gut.", icon: "✦" },
  { time: "T – 5 min", label: "Breathe", desc: "Ignition Breath — 4-2-4 pattern. 10 cycles. Sympathetic activation.", icon: "◠" },
  { time: "T – 0", label: "Consume", desc: "Receptors pre-loaded with ocimene. Vivid lift arrives with present clarity.", icon: "🌿" },
];

const BEVERAGES = [
  {
    name: "Mint Meditation Tea", subtitle: "Cooling mint and warming ginger tea for alert serenity",
    meta: "Teacup · 6oz · 195°F · 4 min · No caffeine",
    ingredients: [
      "6 oz filtered water",
      "1 tbsp fresh mint leaves (bruised)",
      "3-4 slices fresh ginger",
      "2-3 honeysuckle flowers (dried)",
      "0.5 tsp raw honey",
    ],
    accent: "#7ECBA1",
  },
  {
    name: "Mint Lift Mocktail", subtitle: "Fresh mint and ginger sparkle with tropical joy and clarity",
  ...
  {
    name: "Tiki Clarity Cocktail", subtitle: "White rum carries mint, ginger, and grapefruit into euphoric focus",
    meta: "Tiki mug · 7oz · White rum · ~28% ABV",
    ingredients: [
  ...
  {
    name: "Clarity Blend Juice", subtitle: "Cold-pressed mint and ginger with grapefruit brilliance",
    ...
    ingredients: [
      ...
      "1 oz coconut water",
      "1 mint sprig + grapefruit wheel",
    ],
    accent: "#F4D35E",
  },
  {
    name: "Euphoric Rush Shot", subtitle: "A vibrant ginger-mint spark with tropical clarity",
    meta: "Shot · 1.5oz · Ice cold · Consume immediately",
    ingredients: [
      "5 leaves fresh mint (muddled)",
      "1 oz white rum",
      "0.5 oz fresh ginger-grapefruit juice",
      "1 mint leaf + ginger slice (garnish)",
    ],
    accent: "#C85E5E",
  },
];
...
const MUSIC = {
  bpm: "87–111 BPM",
  description: "A bright, uplifting, energetic session. Vivid lift with present clarity. Indie pop, tropical, bossa nova, deep house, reggae — the music meets where you are and builds toward full momentum.",
  artists: ["Jungle", "Tom Misch", "Seu Jorge", "Khruangbin"],
  genres: ["indie pop", "tropical", "bossa nova", "deep house", "reggae"],
};
...
const FROST_PARTICLES = [
  { left: 8, size: 4, delay: 0, dur: 22 },
  { left: 18, size: 3, delay: 4, dur: 28 },
  { left: 30, size: 5, delay: 2, dur: 24 },
  { left: 44, size: 3, delay: 8, dur: 26 },
  { left: 58, size: 4, delay: 5, dur: 20 },
  { left: 68, size: 3, delay: 10, dur: 30 },
  { left: 80, size: 5, delay: 3, dur: 25 },
  { left: 92, size: 4, delay: 7, dur: 27 },
];

const CORAL_GLOWS = [
  { top: 15, left: 12, size: 280, delay: 0, dur: 7 },
  { top: 48, left: 68, size: 340, delay: 2.5, dur: 8 },
  { top: 78, left: 20, size: 260, delay: 4.5, dur: 6 },
];
...
export default function TropicalSleighRideVibe() {
  return (
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        ::-webkit-scrollbar-thumb { background: rgba(232,122,122,0.2); border-radius: 2px; }
      `}</style>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 0 }}>
        {CORAL_GLOWS.map((g, i) => (
          <div key={i} style={{
            ...
            background: `radial-gradient(circle, ${COLORS.accent1}40 0%, ${COLORS.accent2}10 50%, transparent 75%)`,
            filter: "blur(40px)",
            animation: `coralPulse ${g.dur}s ease-in-out infinite`,
      ...
      <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 1 }}>
        {FROST_PARTICLES.map((f, i) => (
          <div key={i} style={{
            position: "absolute", left: `${f.left}%`, top: -20,
            width: f.size, height: f.size, borderRadius: "50%",
            background: "#F0F8FA",
            boxShadow: "0 0 6px rgba(240,248,250,0.7)",
            animation: `frostDrift ${f.dur}s linear infinite`,
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 520, margin: "0 auto", paddingBottom: 80 }}>
        ...
        <div style={{ padding: "36px 28px 28px" }}>
          ...
          <p style={{ fontSize: 14, color: COLORS.textMuted, lineHeight: 1.75, margin: "0 0 24px" }}>
            A Tropical Sleigh Ride session isn't just smoking a joint. You taste the same molecules
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              { channel: "Diffuser", path: "Olfactory → Limbic System", desc: "Basil + black pepper → nose → emotional brain in under 2 seconds" },
              { channel: "Juice / Mocktail", path: "Gut → Bloodstream → Brain", desc: "Mint + ginger + grapefruit terpenes cross the blood-brain barrier" },
              { channel: "Flower", path: "Lungs → Bloodstream → Receptors", desc: "Cannabis ocimene arrives at already-activated receptors" },
            ].map((c, i) => (
              <div key={i} style={{
        ...
        <div style={{
          ...
          <p style={{ fontSize: 14, color: COLORS.textMuted, lineHeight: 1.75, margin: "0 0 20px" }}>
            Every plant that smells — cannabis, basil, black pepper, peppermint, grapefruit, geranium — gets its scent from
          ...
          <p style={{ fontSize: 13, color: COLORS.textDim, lineHeight: 1.6, margin: 0 }}>
            The Tropical Sleigh Ride blend matches its exact terpene fingerprint — rare ocimene from basil, caryophyllene from
            black pepper, limonene from grapefruit, menthol from peppermint — so your body receives the same
        ...
        <div style={{ padding: "0 28px 36px" }}>
          ...
          <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.7, marginBottom: 20, fontStyle: "italic" }}>
            Every ingredient maps to Tropical Sleigh Ride's terpene fingerprint. Basil and mint deliver ocimene.
            Black pepper carries caryophyllene. Grapefruit provides limonene. Ginger brings the warming counterpoint.
        ...
        <div style={{ padding: "0 28px 36px" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
              ...
              <div style={{ fontFamily: FONTS.mono, fontSize: 10, color: COLORS.textDim }}>Energy 0.55 · Valence 0.59</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[
                { phase: "Ignite", bpm: "87", mins: "5" },
                { phase: "Peak", bpm: "111", mins: "15" },
                { phase: "Coast", bpm: "99", mins: "10" },
              ].map(p => (
          ...
          <a
            href="https://open.spotify.com/search/tropical%20indie%20pop%20bossa%20nova%2087%20BPM"
        ...
        <div style={{ padding: "0 28px 40px" }}>
          <SectionLabel>The Narrative · Honeysuckle Flame</SectionLabel>
        ...
        <div style={{
          ...
          <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.65, marginBottom: 20 }}>
            The idea isn't "get high." It's "choose what you want the session to do." Tropical Sleigh Ride lives in three categories:
          </p>
