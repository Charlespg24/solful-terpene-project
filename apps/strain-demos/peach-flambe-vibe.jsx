...
const TERPENES = [
  { name: "β-Caryophyllene", pct: 0.25, ratio: 24, color: "#E8A87C", oil: "Black Pepper", food: "Black Pepper, Cloves, Cinnamon", note: "CB2 agonist — anti-inflammatory warmth with no psychoactivity." },
  { name: "Myrcene", pct: 0.21, ratio: 20, color: "#7ec8a0", oil: "Lemongrass", food: "Mango, Hops, Bay Laurel", note: "Heavy-bodied warmth. Amplifies cannabinoid uptake." },
  { name: "Limonene", pct: 0.20, ratio: 19, color: "#F4D35E", oil: "Bergamot", food: "Citrus Peels, Juniper, Dill", note: "Mood elevation and stress relief. Rapid absorption." },
  { name: "α-Humulene", pct: 0.12, ratio: 11, color: "#A8945A", oil: "Sage", food: "Hops, Sage, Ginseng", note: "Earthy, herbaceous counterweight. Anti-inflammatory partner to caryophyllene." },
];

const EO_BLEND = [
  { name: "Bergamot", drops: 10, terpene: "Limonene", color: "#F4D35E" },
  { name: "Black Pepper", drops: 10, terpene: "β-Caryophyllene", color: "#E8A87C" },
  { name: "Lemongrass", drops: 8, terpene: "Myrcene", color: "#7ec8a0" },
  { name: "Geranium", drops: 6, terpene: "Linalool", color: "#C4A8D4" },
  { name: "Sage", drops: 4, terpene: "α-Humulene", color: "#A8945A" },
  { name: "Clove", drops: 2, terpene: "β-Caryophyllene", color: "#B88A5A" },
];

const BLEND_RATIONALE = "Balanced, lower-terpene profile. Bergamot brings sophisticated citrus matching the peach. Black pepper for caryophyllene. Lemongrass for myrcene. Geranium adds fruity, peachy notes. Sage for humulene. Clove echoes the brown sugar warmth.";

const PROTOCOL_STEPS = [
  { time: "T – 30 min", label: "Diffuse", desc: "Start diffusing bergamot + black pepper. Open space, clean surfaces, bright daylight.", icon: "☀" },
  { time: "T – 15 min", label: "Sip", desc: "Pour the Peach Flambé mocktail or juice. Peach-citrus delivery through the gut.", icon: "✦" },
  { time: "T – 5 min", label: "Breathe", desc: "Ignition Breath — 4-2-4 pattern. 10 cycles. Build momentum.", icon: "◠" },
  { time: "T – 0", label: "Consume", desc: "Receptors pre-loaded with caryophyllene. The drive arrives bright and ready.", icon: "🌿" },
];

const BEVERAGES = [
  {
    name: "Peach Flambé Tea", subtitle: "Bergamot and peach comfort in a cup",
    meta: "Teacup · 8oz · 200°F · 3 min steep · Moderate caffeine",
    ingredients: [
      "8 oz filtered water",
      "1 black tea bag + 1 bergamot tea bag",
      "0.5 oz dried peach pieces (or 0.75 oz puree)",
      "0.25 oz demerara syrup · pinch cinnamon",
      "0.5 oz cashew milk",
      "cinnamon stick + peach slice (garnish)",
    ],
    accent: "#F4D35E",
  },
  {
    name: "Peach Flambé Mocktail", subtitle: "Sunny white peach with warm spice",
  ...
  {
    name: "Peach Flambé Cocktail", subtitle: "Bourbon-backed peach warmth",
    meta: "Coupe · 6.5oz · Bourbon · 18% ABV",
    ingredients: [
      "1.75 oz bourbon",
      "2 oz fresh white peach puree",
      "0.5 oz fresh lemon juice",
      "0.5 oz bergamot-infused syrup",
      "0.25 oz demerara syrup · 0.5 oz cashew milk",
      "flamed cinnamon + grilled peach (garnish)",
    ],
    accent: "#C4885C",
  },
  {
    name: "Peach Flambé Juice", subtitle: "White peach and cashew smoothness",
    meta: "Highball · 8oz · Chilled · Serve immediately",
    ingredients: [
      "2 ripe white peaches (peeled, pitted)",
      "0.75 oz raw cashew butter + 1 oz warm water",
      "0.5 oz fresh lemon juice",
      "0.25 oz demerara syrup",
      "1 oz cold filtered water",
      "peach slice + cinnamon dust (garnish)",
    ],
    accent: "#F0BC8E",
  },
  {
    name: "Peach Flambé Shot", subtitle: "One-sip peachy warmth",
...
const MUSIC = {
  bpm: "78–102 BPM",
  description: "A bright, uplifting, energetic session. Sunny drive with bright momentum. Deep house, reggae, jazz, ambient, dub — tempo meets you low, builds to full momentum, then sustains the glow.",
...
const FLAMES = [
  { left: 8, size: 16, delay: 0, dur: 16, flicker: 1.4 },
  { left: 22, size: 12, delay: 3, dur: 18, flicker: 1.8 },
  { left: 38, size: 20, delay: 1.2, dur: 15, flicker: 1.2 },
  { left: 52, size: 14, delay: 5, dur: 19, flicker: 2.1 },
  { left: 68, size: 18, delay: 2.4, dur: 17, flicker: 1.6 },
  { left: 82, size: 13, delay: 6, dur: 20, flicker: 2.4 },
  { left: 94, size: 15, delay: 4, dur: 16, flicker: 1.9 },
];
...
function Flame({ size, color }) {
  return (
    <svg width={size} height={size * 1.5} viewBox="0 0 20 30" fill="none">
      <path
        d="M10 2 C 6 9, 3 14, 3 20 C 3 25.5, 6 29, 10 29 C 14 29, 17 25.5, 17 20 C 17 14, 14 9, 10 2 Z"
        fill={color}
        opacity="0.7"
      />
      <path
        d="M10 8 C 8 12, 7 15, 7 20 C 7 23.5, 8.5 26, 10 26 C 11.5 26, 13 23.5, 13 20 C 13 15, 12 12, 10 8 Z"
        fill={COLORS.gold}
        opacity="0.9"
      />
    </svg>
...
export default function PeachFlambeVibe() {
  return (
    <div style={{
      ...
      <style>{`
        @keyframes riseFlame {
          0% { transform: translateY(100vh) scale(0.7) rotate(-2deg); opacity: 0; }
          10% { opacity: 0.5; }
          25% { transform: translateY(70vh) scale(0.9) rotate(3deg); }
          50% { transform: translateY(40vh) scale(1) rotate(-3deg); }
          75% { transform: translateY(15vh) scale(0.95) rotate(2deg); opacity: 0.4; }
          100% { transform: translateY(-20vh) scale(0.6) rotate(0deg); opacity: 0; }
        }
        @keyframes flicker {
          0%, 100% { opacity: 0.55; transform: scaleY(1) scaleX(1); }
          20% { opacity: 0.85; transform: scaleY(1.15) scaleX(0.92); }
          40% { opacity: 0.45; transform: scaleY(0.9) scaleX(1.08); }
          60% { opacity: 0.75; transform: scaleY(1.08) scaleX(0.95); }
          80% { opacity: 0.6; transform: scaleY(0.96) scaleX(1.04); }
        }
        * { box-sizing: border-box; }
        ::-webkit-scrollbar { width: 4px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: rgba(232,168,122,0.2); border-radius: 2px; }
      `}</style>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 0 }}>
        {FLAMES.map((f, i) => (
          <div key={i} style={{
            ...
            animation: `riseFlame ${f.dur}s linear infinite`,
            animationDelay: `${f.delay}s`,
          }}>
            <div style={{
              animation: `flicker ${f.flicker}s ease-in-out infinite`,
              transformOrigin: "bottom center",
            }}>
              <Flame size={f.size} color={i % 2 === 0 ? COLORS.accent1 : COLORS.accent2} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 520, margin: "0 auto", paddingBottom: 80 }}>
        ...
        <div style={{ padding: "36px 28px 28px" }}>
          ...
          <p style={{ fontSize: 14, color: COLORS.textMuted, lineHeight: 1.75, margin: "0 0 24px" }}>
            A Peach Flambé session isn't just smoking a joint. You taste the same molecules
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              ...
              { channel: "Juice / Mocktail", path: "Gut → Bloodstream → Brain", desc: "White peach + bergamot terpenes cross the blood-brain barrier" },
        ...
        <div style={{
          ...
          <p style={{ fontSize: 14, color: COLORS.textMuted, lineHeight: 1.75, margin: "0 0 20px" }}>
            Every plant that smells — cannabis, bergamot, black pepper, lemongrass, geranium — gets its scent from
          ...
          <p style={{ fontSize: 13, color: COLORS.textDim, lineHeight: 1.6, margin: 0 }}>
            The Peach Flambé blend matches its exact terpene fingerprint — caryophyllene from black pepper and clove,
            myrcene from lemongrass, limonene from bergamot, humulene from sage — so your body receives the same
        ...
        <div style={{ padding: "0 28px 36px" }}>
          ...
          <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.7, marginBottom: 20, fontStyle: "italic" }}>
            Every ingredient maps to Peach Flambé's terpene fingerprint. Bergamot delivers limonene. Black pepper and
            clove carry caryophyllene. Lemongrass provides myrcene. Cashew milk and demerara echo the brown sugar base.
          </p>
        ...
        <div style={{ padding: "0 28px 36px" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[
                { phase: "Ignite", bpm: "78", mins: "5" },
                { phase: "Peak", bpm: "102", mins: "15" },
                { phase: "Coast", bpm: "90", mins: "10" },
              ].map(p => (
          ...
          <a
            href="https://open.spotify.com/search/peach%20flambe%20deep%20house%20reggae%20jazz"
        ...
        <div style={{ padding: "0 28px 40px" }}>
          <SectionLabel>The Narrative · Cyclist's Climb</SectionLabel>
        ...
        <div style={{
          ...
          <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.65, marginBottom: 20 }}>
            The idea isn't "get high." It's "choose what you want the session to do." Peach Flambé lives in three categories:
          </p>
