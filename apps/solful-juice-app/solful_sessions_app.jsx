...
function HomePage({onVibe,onStrain,onNav}){
  const featured=[S[8],S[11],S[13]]; // Carambola, Pink Rider, Avenue
  const arc=[S[8],S[11],S[9]]; // Higher Heights daily: Carambola→Pink Rider→Pineapple Mojito
  const vibeGrid=[
    {v:"Creative Flow",emoji:"✨",c:"#EF9F27"},
    {v:"Deep Rest",emoji:"🌙",c:"#9B7BA8"},
    {v:"Social & Bright",emoji:"☀️",c:"#D4C85C"},
    {v:"Body Melt",emoji:"🫠",c:"#8EAE68"},
    {v:"Euphoric Lift",emoji:"🎈",c:"#E87A90"},
    {v:"Cozy Comfort",emoji:"🕯️",c:"#C8A97E"},
    {v:"Grounded & Present",emoji:"🌲",c:"#5C8E6B"},
    {v:"Calm Focus",emoji:"🎯",c:"#85B7EB"},
  ];

  return <div>
    <div style={{background:`linear-gradient(180deg, ${P.dark} 0%, #243824 100%)`,padding:"40px 24px 36px",textAlign:"center"}}>
      <div style={{fontSize:13,color:`${P.gold}90`,letterSpacing:3,textTransform:"uppercase",marginBottom:8}}>Solful Sessions</div>
      <div style={{fontSize:32,fontWeight:600,fontFamily:"Georgia,serif",color:P.gold,lineHeight:1.2,marginBottom:8}}>Terpene Juice<br/>Recipe Library</div>
      <div style={{fontSize:14,color:"#ffffffaa",lineHeight:1.6,maxWidth:320,margin:"0 auto"}}>24 cannabis strains matched to botanical juice recipes through shared terpene chemistry</div>
      <div style={{display:"flex",justifyContent:"center",gap:20,marginTop:20}}>
        {[["24","Strains"],["14","Farms"],["10","Terpenes"]].map(([n,l],i)=>
          <div key={i} style={{textAlign:"center"}}>
            <div style={{fontSize:22,fontWeight:600,color:"#fff"}}>{n}</div>
            <div style={{fontSize:10,color:"#ffffff60",textTransform:"uppercase",letterSpacing:1}}>{l}</div>
          </div>
        )}
      </div>
    </div>
    ...
    <div style={{padding:"0 16px",marginTop:-16}}>
      <div style={{background:P.card,borderRadius:14,padding:"16px 18px",boxShadow:P.shadow,borderLeft:`3px solid ${P.gold}`}}>
        <div style={{fontSize:15,fontFamily:"Georgia,serif",color:P.text,lineHeight:1.6,fontStyle:"italic"}}>"The same molecule activates the same receptor regardless of botanical origin."</div>
        <div style={{fontSize:12,color:P.text3,marginTop:8}}>β-caryophyllene from black pepper and from cannabis both activate CB2. Limonene from lemon peel and from flower both modulate serotonin. Every juice recipe in this library is built on that principle.</div>
      </div>
    </div>

    <div style={{padding:"24px 16px 0"}}>
      <div style={{fontSize:11,fontWeight:600,color:P.gold,textTransform:"uppercase",letterSpacing:1.5,marginBottom:12}}>What's your vibe?</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2, 1fr)",gap:8}}>
        {vibeGrid.map(({v,emoji,c})=>
          <button key={v} onClick={()=>onVibe(v)} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderRadius:12,border:"none",background:P.card,boxShadow:"0 1px 6px rgba(0,0,0,0.04)",cursor:"pointer",textAlign:"left"}}>
            <span style={{fontSize:20}}>{emoji}</span>
            <div>
              <div style={{fontSize:13,fontWeight:500,color:P.text}}>{v}</div>
              <div style={{fontSize:10,color:P.text3}}>{S.filter(s=>s.vb.includes(v)).length} strains</div>
            </div>
          </button>
        )}
      </div>
    ...
    <div style={{padding:"28px 16px 0"}}>
      <div style={{fontSize:11,fontWeight:600,color:P.gold,textTransform:"uppercase",letterSpacing:1.5,marginBottom:4}}>Higher Heights Daily Arc</div>
      <div style={{fontSize:12,color:P.text3,marginBottom:12}}>Morning → afternoon → evening from one Mendocino farm</div>
      <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4,WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
        {[
          {s:arc[0],time:"Morning",note:"Limonene-led citrus lift"},
          {s:arc[1],time:"Afternoon",note:"Terpinolene creative spark"},
          {s:arc[2],time:"Evening",note:"7-terpene grounded glow"},
        ].map(({s,time,note},i)=>
          <div key={i} onClick={()=>onStrain(s.id)} style={{minWidth:160,background:P.card,borderRadius:12,overflow:"hidden",boxShadow:P.shadow,cursor:"pointer",flexShrink:0}}>
            <div style={{height:6,display:"flex"}}>
              {s.tp.map((t,ti)=><div key={ti} style={{width:`${t.p}%`,background:T[t.n]||"#ccc"}}/>)}
            </div>
            <div style={{padding:"10px 12px"}}>
              <div style={{fontSize:10,color:P.gold,fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>{time}</div>
              <div style={{fontSize:14,fontWeight:500,fontFamily:"Georgia,serif",color:P.text,marginTop:2}}>{s.nm}</div>
              <div style={{fontSize:11,color:P.text3,marginTop:2}}>{note}</div>
            </div>
          </div>
        )}
      </div>
    ...
    <div style={{padding:"28px 16px 0"}}>
      <div style={{fontSize:11,fontWeight:600,color:P.gold,textTransform:"uppercase",letterSpacing:1.5,marginBottom:12}}>Featured strains</div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {featured.map(s=>
          <div key={s.id} onClick={()=>onStrain(s.id)} style={{display:"flex",gap:12,background:P.card,borderRadius:12,overflow:"hidden",boxShadow:P.shadow,cursor:"pointer"}}>
            <div style={{width:80,flexShrink:0,position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",inset:0,background:s.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <svg viewBox="0 0 80 80" width="80" height="80">
                  <rect width={80} height={80} fill={s.bg}/>
                  <circle cx={40} cy={40} r={25} fill={s.c1} opacity={0.15}/>
                  <rect x={28} y={18} width={24} height={42} rx={3} fill={s.c1} opacity={0.25}/>
                  <rect x={28} y={18} width={24} height={42} rx={3} fill="none" stroke={s.c1} strokeWidth={0.5} opacity={0.3}/>
                  <rect x={31} y={28} width={18} height={30} rx={2} fill={s.c1} opacity={0.3}/>
                  <rect x={31} y={40} width={18} height={18} rx={2} fill={s.c2} opacity={0.15}/>
                </svg>
              </div>
            </div>
            <div style={{padding:"10px 12px 10px 0",flex:1,minWidth:0}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:15,fontWeight:500,fontFamily:"Georgia,serif",color:P.text}}>{s.nm}</span>
                <span style={{display:"flex",alignItems:"center",gap:3,fontSize:10,color:P.text3}}>
                  <span style={{width:6,height:6,borderRadius:6,background:EC[s.en]}}/>
                  {s.en}
                </span>
              </div>
              <div style={{fontSize:11,color:P.text3,marginTop:1}}>{s.fm}</div>
              <div style={{fontSize:12,color:P.text2,fontStyle:"italic",marginTop:4,fontFamily:"Georgia,serif"}}>{s.in}</div>
    ...
    <div style={{padding:"28px 16px 0"}}>
      <div style={{background:`linear-gradient(135deg, ${P.dark}, #2A4A2A)`,borderRadius:14,padding:"20px 18px",textAlign:"center"}}>
        <div style={{fontSize:11,color:P.gold,textTransform:"uppercase",letterSpacing:2,marginBottom:8}}>The science</div>
        <div style={{fontSize:15,color:"#ffffffdd",lineHeight:1.6,fontFamily:"Georgia,serif",marginBottom:12}}>Cannabis and culinary herbs share the same terpene molecules. Each juice recipe matches a strain's unique terpene fingerprint using botanical ingredients that deliver the same compounds.</div>
        <div style={{display:"flex",justifyContent:"center",gap:6,flexWrap:"wrap"}}>
          {["β-Caryophyllene","Limonene","Myrcene","Linalool","α-Pinene"].map(t=>
            <span key={t} style={{fontSize:10,padding:"3px 8px",borderRadius:8,background:`${T[t]}25`,color:T[t],fontWeight:500}}>{t}</span>
          )}
        </div>
      </div>
    </div>
    ...
    <div style={{padding:"24px 16px"}}>
      <button onClick={()=>onNav("recipes")} style={{width:"100%",padding:"14px",borderRadius:12,border:"none",background:P.gold,color:"#fff",fontSize:15,fontWeight:600,cursor:"pointer",boxShadow:"0 2px 10px rgba(196,148,61,0.3)"}}>
        Explore all 24 recipes →
      </button>
    </div>

    <div style={{padding:"0 16px 24px"}}>
      <div style={{background:P.bg2,borderRadius:12,padding:"14px 16px"}}>
        <div style={{fontSize:11,fontWeight:600,color:P.text3,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>About this project</div>
        <div style={{fontSize:12,color:P.text2,lineHeight:1.6}}>Solful Sessions is a multisensory cannabis experience platform developed in partnership with Solful Dispensary. Each strain is profiled across 15 sensory layers — from essential oil blends to sonic playlists to these terpene-matched juice recipes. Created by Charles Pelletier-Gagné.</div>
      </div>
    </div>
  </div>;
}
...
export default function App(){
  ...
  const header=page!=="home"?<div style={{background:P.dark,padding:"14px 20px",textAlign:"center"}}>
  ...
  const tabBar=<div style={{position:"fixed",bottom:0,left:0,right:0,background:P.dark,display:"flex",justifyContent:"center",zIndex:20,maxWidth:640,margin:"0 auto",borderTop:"1px solid #ffffff10"}}>
    {[["home","⌂","Home"],["recipes","✦","Recipes"],["ingredients","◈","Ingredients"]].map(([k,icon,l])=>
  ...
  return <div style={{maxWidth:640,margin:"0 auto",background:P.bg,minHeight:"100vh",paddingBottom:80}}>
    ...
    {page==="home"&&<HomePage onVibe={(v)=>{setVb(v);setPage("recipes");}} onStrain={(id)=>{setSel(id);setPage("recipes");}} onNav={(p)=>setPage(p)}/>}
